use diet_code_core::edits::{apply_symbol_removals, prune_unused_imports, remove_byte_range, SymbolRemoval};
...
fn removes_full_declaration_lines() {
    ...
    assert!(!out.contains("dead"), "dead declaration must be gone:\n{}", out);
    assert!(out.contains("export function used()"), "unrelated code untouched:\n{}", out);
    assert!(out.contains("export const y"), "trailing code untouched:\n{}", out);
    // No half-lines remain.
    for line in out.lines() {
        assert!(!line.starts_with('}') || line == "}", "no orphan braces: {:?}", line);
    }
}
...
fn applies_multiple_removals_stably() {
    ...
    assert!(!out.contains("function a()") && !out.contains("function c()"), "a/c removed:\n{}", out);
}
...
fn prunes_only_proven_unused_imports() {
    ...
    assert!(!out.contains("./y"), "fully unused default import pruned:\n{}", out);
    assert!(out.contains("./side-effect"), "side-effect import kept:\n{}", out);
}
